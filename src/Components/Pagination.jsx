		...
		<div>
			...
			{arrayPag(pages).map((pg) => {
					if(current == pages)return (pg+1 > pages-3) && (pg+1 === current ? 
						(<span style={{marginLeft:'10px',marginRight:'10px',borderRadius:'50px',padding:'10px',background:'#fefefe',color:'#000000'}}>{pg+1}</span>)
						:
						(<span onClick={() => setPage(pg+1)} style={{cursor:'pointer', marginLeft:'10px',marginRight:'10px',borderRadius:'50px',padding:'10px'}}>{pg+1}</span>)
						)
			...
			<button disabled={next == 1} onClick={() => setPage(next)}>next</button>
		</div>
	)
}
